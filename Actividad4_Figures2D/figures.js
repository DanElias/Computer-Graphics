...
* Year: 2020
* Title: WebGL - 2D - Pipeline and Drawing Primitives
*/
...
function createSquare(gl, width) 
{
    let vertexBuffer;
...
function createTriangle(gl, length)
{
    let vertexBuffer;
    vertexBuffer = gl.createBuffer();
    gl.bindBuffer(gl.ARRAY_BUFFER, vertexBuffer);
...
function createPacMan(gl, radius)
{
    ...
    let center_x = 0;
    let center_y = 0; 
    let number_of_fans = 100;
    let pacman_mouth = number_of_fans/6; //1/6 del círculo no se va a dibujar
    let angle_per_fan = (2*Math.PI) / number_of_fans;
    let initial_angle = Math.PI/6;
    //(x,y,z) = ( r * cos(theta), r * sin(theta), 0)

    verts.push(center_x);
    verts.push(center_y);
    for (let i = 0; i < number_of_fans - pacman_mouth ; i++){
        let angle = angle_per_fan * i + initial_angle;
    ...
    // The resulting object contains the vertexbuffer, the size of the vertex structure (3 floats, x, y, z), the number of vertices to be drawn, the the primitive to draw.
    let pac_man = {buffer:vertexBuffer, vertSize: 2, nVerts: verts.length / 2, primtype:gl.TRIANGLE_FAN};
    return pac_man;
}  
