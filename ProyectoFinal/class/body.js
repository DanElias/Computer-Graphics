...
class Body{
    constructor(radius, position, rotation, rotation_speed, translation_speed, color, material, num_moons, hasRing, orbit_radius, parent_group, isStar, isAsteroid, name){
        ...
        Game.scene.add(this.orbit_group) //add to scene and not to parent
    }
    ...
    addMoons(){
        ...
        for(let i = 1; i <= this.num_moons; i++){
            //set material
            let material_moon = Game.materials.moon;
            //special moons for mars
            if(this.name == "mars"){
                if(i-1 == 0) material_moon = Game.materials.phobos;
                else material_moon = Game.materials.deimos;
            }
